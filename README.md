...
<div align="center">
  <img src="./frontend/public/logo.png" alt="StellarTradeAgent Logo" width="200" />
</div>
...
We believe in full transparency about what this project does and does not do.

### What is REAL (On-Chain, Verifiable)

Every financial interaction in this system is a **real Stellar Testnet transaction** — not mocked, not simulated:

| Action | Protocol | Verifiable |
|--------|----------|------------|
| Price data payments (0.01 USDC each) | MPP via Soroban SAC | ✅ Every `txHash` on [stellar.expert](https://stellar.expert) |
| Intelligence payments (0.01 USDC each) | x402 via Facilitator | ✅ Every `txHash` on stellar.expert |
| Trade execution (BUY/SELL) | SDEX `manageBuyOffer` / `manageSellOffer` | ✅ Every `txHash` + AI reasoning in TX memo |
| Wallet balances | Horizon API | ✅ Live query to Stellar Testnet |

### What is SIMULATED

The **market price feed** is generated by a local `price-engine.js` — a controlled market simulator that produces realistic Wyckoff-style market cycles (Accumulation → Breakout → Rally → Distribution → Correction). It does **not** pull live XLM/USDC prices from CoinGecko or any mainnet exchange.

**Why?** This project's goal is to demonstrate **Machine-to-Machine commerce architecture** on Stellar — proving that an AI agent can autonomously pay for data, reason about it, and execute trades using official Stellar SDKs. The price source is intentionally controlled so we can reliably demonstrate the full on-chain payment pipeline in a 2-minute live demo without being subject to flat mainnet market conditions.

### What This Means for Win Rate & PnL

The 100% Win Rate and +$16.85 PnL are **mathematically honest** — the P&L formula `(exitPrice - entryPrice) × amount` is applied without manipulation. However, these results reflect performance against a **controlled price feed**, not against real unpredictable market conditions. In a mainnet environment with genuine market chaos, slippage, and liquidity constraints, a realistic win rate would be closer to 40–60%.

**The innovation is not the trading strategy — it's the autonomous economic infrastructure.** The agent paying for its own data, reasoning on-chain, and executing trades via native Stellar protocols is what makes this project unique.
